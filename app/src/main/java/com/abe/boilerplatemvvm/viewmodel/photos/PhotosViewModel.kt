...
//    var photosListLiveData: LiveData<List<PhotoModel>> = _photosList
    ...
    private lateinit var _allPhotosLiveData: MutableLiveData<StateViewModel<PhotoModel>>
    private var allPhotosResultLiveData: MediatorLiveData<StateViewModel<PhotoModel>> = MediatorLiveData()
...
//        _uiState.postValue(if (page == 1) LoadingState else LoadingNextPageState)
        if (this::_allPhotosLiveData.isInitialized)
            allPhotosResultLiveData.removeSource(_allPhotosLiveData)
//        _allPhotosLiveData =
//                fetchPopularPhotosUseCase(page, job)
