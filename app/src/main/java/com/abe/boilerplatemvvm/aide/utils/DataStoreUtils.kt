...
object DataStoreUtils {
    ...
    fun init(context: Context) {
        dataStore = context.createDataStore(KEY_PREF_NAME)
    }
    ...
    fun <U> getSyncData(key: String, default: U): U {
        val res = when (default) {
            ...
            else -> throw IllegalArgumentException(TYPE_EXCEPTION)
        }
        return res as U
    }
    ...
    fun readBooleanFlow(key: String, default: Boolean = false): Flow<Boolean> =
        dataStore.data
            .catch {
                //When reading data encounter errors，in the case of `IOException` abnormal，Send one emptyPreferences To reuse
                //But if it is other abnormal，It's better to throw it away，Don't hide the problem
